...
(function($) {
	...
	skel.init({
		reset: 'full',
		breakpoints: {
			'desktop':	{ range: '737-', href: base_url + 'css/style-desktop.css', containers: 1200, grid: { gutters: 25 } },
			'1000px':	{ range: '737-1200', href: base_url + 'css/style-1000px.css', containers: 960, grid: { gutters: 25 }, viewport: { width: 1080 } },
			'mobile':	{ range: '-736', href: base_url + 'css/style-mobile.css', containers: '100%!', grid: { collapse: true, gutters: 15 }, viewport: { scalable: false } }
		}
	});
...
(function ($, window) {
    ...
    function Constellation (canvas, options) {
        var container = $('#footer-container').clone().find('#contact').remove().parents('#footer-container');
        var $canvas = $(canvas),
